...
2026-04-28 unknown — [docs] Session 50 PM — Bundle FIX/B/F+G shipped + 14 knowledge promus vault + disk resize +50G + Docker move sdb + /handoff user-level smart auto-detect cross-projet
2026-04-30 Claude — [feat] Session 53 (2026-04-30, ~12h continu) — Library ingest 13 stubs + skim batch 74/85 (87% library) + EN/ES authority anchor audit Phase A+B+C+D1 LIVRÉE — curriculum_en 105→131 (Hawkins criterial + Companion 2020) + curriculum_es 51→98 (PCIC Gramática A1-B2) + Functions dimension scaffold (Pydantic+loader+ES PCIC Funciones+EN stub) + 5 authority anchors extracted (Hawkins Ch 9.1 + Companion App 1+7 + PCIC Vol A+B Gramática). Oracle full EN 17/26 (baseline 18-19/26 ±1, dans plage). 17 commits academia + ~17 vault.

2026-04-30 claude — [feat] Sprint Oracle EN cohérence MVP COMPLETE — Phase 0-6 + extensions (Session 54). Capacity unlock 540→166K RPD (Cerebras+Mistral), multi-judge panel cross-provider (3 judges), Gwet AC2 inter-run, κ Opus vs panel 0.85/1.0/1.0, cf_move judge prompt v2 (CoT+7 few-shots), 12 scenario specs Lyster-aligned, verdict cache 4× speedup. 49/49 oracle tests green. Architecture trustworthy + multi-langue ready → switch Maestro ES authorized. 10 commits : 0d721ec, d1eb6ec, 9f0a77c, fc54394, 19d9ffa, 31f13b6, 186543a, 6f58148, a536f4c, 552e55e

2026-05-01 Claude — [feat] Session 56 (2026-05-01, ~16h continu) — Maestro ES MVP-ACCEPTABLE end-to-end : Tier 1 prompt fix v1+v2+v3 (18/24 scope-cap) + Tier 2-3 data layer (curriculum 51→138, hints 20→143, functions/es 0→75, functions/en 5→41, PCIC Vol A+B+C extracted, DELE A1-C1 rubrics structurels, DELE Criterios 4 dims) + Tier 4 rules_es +12 codes V:* + l1_transfer 14→30 + FP whitelists + Tier 5 oracle 24→31 scenarios (4 C1 + 3 B2 thematic) + 5 ES fewshots CF_MOVE_PROMPT + Tier 6 RE-MEASURE FINAL battery 23/31 = 74.2% (κ Opus 0.93/0.81/0.93 ATTEINT, AC2 panel internal variance acknowledged). Wave 2-4 IT/DE/JP/RU AUTHORIZED. 26 commits academia.

2026-05-02 Claude — [feat] Session 57 (2026-05-01/02, ~11h continu cross-jour) — Maître Comptable Mode B livré end-to-end (premier domaine non-linguistique d'AcademIA) + dette tech a11y/oracle/dependabot + roadmap S58-P5. Bloc 0 dette tech matin : restic restore drill réel sha256 prod=restored MATCH ✅, smoke-test 6 vérifs Grafana-lite, INDEX.md audit post-S13 drift, oracle ruff 24→0 + mypy 23→0 + 49/49 tests, a11y WCAG AA fix text-muted contrast 2.4:1→4.7:1, dependabot postcss/cookie 0 vulns. Bloc 1 ADR-017 acted dual-mode (14 décisions D1-D14) + doc compta v3 + 6 agents recherche dispatched. Bloc 2 knowledge base 22 PDFs téléchargés ~36 MB cosmos (PCG v2026 + DCG 9 + DSN cahier 2026.1.2 + URSSAF v4.9 + Sage docs + CNIL × 3 + RNCP41653 + 4 Anna's). Bloc 3 backend AccountingDomain Protocol + 5 tools déterministes + REST endpoints `/internal/compta/tools/*` (57 tests green). Bloc 4 frontend agent maitre_comptable + flag SVG + RGPDDisclaimer + hotfixes UI (skip QCM gate compta + hide UI lang-specific + RGPD onMount fix + remove dropdown Mode B Q&A omniscient + SW cache bump). Bloc 5 Dify chatflow `4ce8ffe2-0cdf-4fa8-aab4-478e5dd8ac1c` créé via API automation (login console base64 + cookies session + DSL workflow draft + hash sync + publish), gpt-4o-mini + vision multimodal, test live "Saisie facture EDF 120€ TTC TVA 20%" → 510 chars cohérent + scaffold Lyster + 740 tokens. Bloc 6 roadmap sprint S58-P5 documentée. ~17 commits academia + 1 sinse-tools.

2026-05-02 Claude — [feat] Session 58 (2026-05-02, ~9h) — Onboarding Marie + RAG knowledge base 22 PDFs livré end-to-end + profile-building meta. Bloc 0 profile-building : memory `user_profile_sinse.md` (9 ajustements collaboration push-back/doute/jargon/erreurs/symétrie), plan audit infrastructure archivé `vault/meta/plan-audit-infrastructure-2026-05.md`, /pickup skill cleanup fossile memory-mirror-last (efeb706 sinse-claude-config). Bloc 1 onboarding Marie ✅ : compte `mariejuanes` créé via POST /api/auth/users + CF Zero Trust policy email Marie ajouté manual dashboard (gotcha S47 confirmé). Bloc 2 RAG end-to-end ⭐ : LiteLLM enrichi (gemini-embed + openai-embed-small routes), Qdrant container démarré (jamais up auparavant, persistent volume /mnt/cosmos-data/qdrant-data, academie-net-bridge), Dify env QDRANT_URL fix (recreate dify-api+dify-worker), Dify provider register openai-embed-small, Dify dataset `79ab2618-5762-465d-9fab-b5ed54cff214` créé (openai-embed-small 1536 dims), 22 docs uploaded + indexed = **15,689 chunks Qdrant + 5.86M words searchable** (DCG 9 22.9MB split en 3 ghostscript + Compta Nuls EPUB→.txt calibre + PCG Recueil 2.8M dropped redondant), 4 metadata fields créés (authority_priority, domain, valid_from, bc) + 22/22 docs assigned, Knowledge Retrieval node wired chatflow `4ce8ffe2` (start→knowledge_compta→llm_maitre→answer), test live retrieval validé score 0.78 sur "bulletin de paie" workflow trace. Decisions clés : D-S58.1 Gemini→OpenAI switch (RPD quota exhausted), D-S58.2 PCG Recueil dropped, D-S58.4 test live Marie 12 questions DEFERRED post-P1 (anti-first-impression-burn). Coût ingestion ~$0.04 OpenAI text-embedding-3-small.

2026-05-06 Claude — [refactor] Session 61 — rename /opt/academie → /opt/academia (Option B Cosmétique) — 142 files sed
2026-05-06 Claude — [infra] Session 61 — DNS + CF Zero Trust 5 apps petit-pont (academia/marie/coach/sinse + bypass)
2026-05-06 Claude — [infra] Session 61 — PgBouncer mix mode deploy + asyncpg statement_cache_size=0 patch + Postgres tune (max_conn 150, shared_buffers 1GB)
2026-05-06 Claude — [refactor] Session 61 — Phase 2.3 + 3 — Dify chatflow rerouted to marie-api + academia compta cleanup (-1991 lignes)
2026-05-06 Claude — [infra] Session 61 — save nginx :8080 reverse-proxy config to repo (DR backup)
