...
- **Dify onboarding-branch limitation** (Session 33 hotfix) : la branche `llm_onboarding` bypasse `code_turn_check` ET refuse les refs `{{#start.X#}}` aussi. Les nouveaux users ne bénéficient PAS de l'injection profil au turn 1. Users retournants (via `llm_session`/`llm_plan_choice`) oui. Script revert : `scripts/sprint5/12_revert_llm_onboarding_prepend.py` (appliqué 2026-04-20). **Fix à venir** : wirer via le node upstream de `llm_onboarding` ou restructurer le graph.
