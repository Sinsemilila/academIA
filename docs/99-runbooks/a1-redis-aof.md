...
**Status** : ✅ permanent — container recréé 2026-04-23 11:12 avec `--appendonly yes` baked in. 17 clés préservées via volume `7f51671...`, sinse non déconnecté.
...
## Recreate exécuté (2026-04-23 11:12)

```bash
docker exec redis-academie redis-cli BGREWRITEAOF   # force fsync préalable
...
**Vérifs post-recreate** :
- `DBSIZE` = 17 (= avant recreate, toutes sessions préservées)
- `INFO persistence | grep aof_enabled` → `aof_enabled:1`
- `CONFIG GET appendonly` → `yes` (permanent via Cmd args, plus seulement live setting)
- smoke-test 15/15 + sinse cookie session resté valide (pas de re-login nécessaire)
