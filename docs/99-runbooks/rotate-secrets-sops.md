...
Certains secrets du bundle `secrets/shared.yaml.sops` sont des copies de référence — la source canonique est ailleurs. Quand tu rotates la source canonique, **rotate aussi ici** pour maintenir la parité :
...
SOPS_AGE_KEY_FILE=... sops set --input-type yaml --output-type yaml \
  secrets/shared.yaml.sops '["pg-password"]' '"'"$NEW_VALUE"'"'
