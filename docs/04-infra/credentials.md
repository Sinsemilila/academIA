...
> **Session 62 (2026-05-06) — petit-pont-infra split.** Bundle SOPS canonical déplacé : `/opt/academia/secrets/shared.yaml.sops` → `/opt/petit-pont-infra/secrets/shared.yaml.sops`. Decrypted runtime files restent `/opt/academie-shared/secrets/<name>`. Voir runbook [`99-runbooks/rotate-secrets-sops.md`](../99-runbooks/rotate-secrets-sops.md) (paths à jour).
...
**Phase familiale (maintenant → +6 mois)** : adopter **SOPS** avec clé age. Secrets chiffrés dans git (`petit-pont-infra/secrets/shared.yaml.sops` post-S62, ex `/opt/academia/secrets/shared.yaml.sops`), déchiffrés au démarrage des containers via init script. Rotation facile.
