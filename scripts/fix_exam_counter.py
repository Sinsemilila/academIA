#!/usr/bin/env python3
"""
Add edge from llm_exam to var_assigner so nb_interactions increments during exam.
"""
...
def patch_graph(graph):
    ...
    for e in edges:
        if e.get("source") == "llm_exam" and e.get("target") == "var_assigner":
            print("  Already exists: llm_exam -> var_assigner")
            return graph

    # Add edge: llm_exam -> var_assigner
    edges.append({
        "id": "edge-llmexam-to-varassigner",
        "source": "llm_exam",
        "target": "var_assigner",
        "type": "custom",
        "data": {"sourceType": "llm", "targetType": "assigner"}
    })
    print("  Added edge: llm_exam -> var_assigner")
