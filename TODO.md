...
- [ ] **A6 followup Sinse manuel** — signer DPA OpenAI ([platform.openai.com/account/data-processing-addendum](https://platform.openai.com/account/data-processing-addendum)) + Groq ([groq.com/dpa/](https://groq.com/dpa/)) + désigner `dsar@` Cloudflare Email Routing.

**Manuel à toi (Sinse)** :
- [ ] DMARC bump à `p=quarantine` après 2 sem collecte clean (jalon **2026-05-07**).
- [ ] A3 CSP analyse logs (`csp_violations_24h` view) puis flip `Content-Security-Policy-Report-Only` → `Content-Security-Policy` enforce dans `hooks.server.ts` (jalon **2026-05-07**).
...
- [ ] Cloudflare Email Routing setup (`security@` + `dmarc-reports@` + `dsar@` → forwarding vers ton mail) — token CF account créé Session 47 a maintenant les perms ; modifie DNS MX + écrase SPF, attend OK explicite avant exec.
